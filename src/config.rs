...
pub const APP_VERSION_STR: &str = "3.14";
pub const CONFIG_VERSION_STR: &str = "2.0";
pub const CONFIG_PREDEFINES_FILE: &str = "mmvj_config_predefines.yaml";
...
pub const APP_DEFAULT_CONFIG_FILE: &str = "mmvj_cfg.yaml";
pub const APP_DEFAULT_LATENCY_STR: &str = "normal";
pub const APP_DEFAULT_UPDATE_FREQ_HZ_STR: &str = "1000";
...
pub(crate) struct ConfigManager {
    ...
    control_range_cache: Mutex<HashMap<String, Option<NumInterval<i32>>>>,
}
...
impl ConfigManager {
    ...
    pub(crate) fn load(&mut self) -> Result<()> {
        ...
        if !self.config_file.exists() {
            ...
            let yaml_merged = merge_keys_serde(yaml)
                .map_err(|e| anyhow::anyhow!("Failed to apply YAML merges in config: {e}"))?;

            let merged_str = serde_yaml::to_string(&yaml_merged)?;

            let merged_value = serde_yaml::from_str(&merged_str);
            match merged_value {
                Ok(merged_value) => self.config = merged_value,
                Err(e) => {
                    let line = e.location().unwrap().line();
                    log::error!("{e:?}");
                    self.print_config_read_error_context(&merged_str, line.saturating_sub(5), 20);
                    bail!(e);
                }
            }
        }
