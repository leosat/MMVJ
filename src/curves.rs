...
impl Curves {
    pub(crate) fn apply_center_symmetric_with_abs_value<
        T: crate::num_interval::NumIntervalValue + num_traits::Float,
    >(
        v: T,
    ...
    pub(crate) fn exp_curve(x: BaseNumT, mut base: BaseNumT) -> BaseNumT {
        // TODO: verify during config validation and debug_assert here.
        if base <= 1.0 {
            log::error!(
                "Exponential curve base ({}) must be more than 1.0, config validation should be broken!",
                base
            );
    ...
    pub(crate) fn signed_power(x: BaseNumT, power: BaseNumT) -> BaseNumT {
        // TODO: verify during config validation and debug_assert here.
        if power <= 0.0 {
            log::error!("Power must be > 0.0, config validation should be broken!");
        }
...
mod tests {
    ...
    #[test]
    fn test_power() {
        let result = Curves::signed_power(0.5, 2.0);
        ...
        assert_eq!(
            Curves::apply_center_symmetric_with_abs_value(
                0.5,
                UNIT_INTERVAL,
                |v_abs| { Curves::signed_power(v_abs, 42.0) },
                OutOfRangePolicy::Allow,
            ),
            0.5
        );
        ...
        assert_eq!(
            Curves::apply_center_symmetric_with_abs_value(
                0.75,
                UNIT_INTERVAL,
                |v_abs| { Curves::signed_power(v_abs, 2.0) },
                OutOfRangePolicy::Allow,
            ),
            0.625
        )
