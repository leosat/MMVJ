...
use crate::schemas::VirtualJoystick as JoystickConfig;
...
impl VirtualJoystick {
    ...
    fn ff_consumer_thread(
        ...
        let ff_value_span_f32 =
            crate::common::NumInterval::new(FfLevelT::MIN, FfLevelT::MAX).span_w() as f32;
        // -----------------------------------------------------
        // FFB player routine.
        // TODO: it's very simplified now, single effect played
        // TODO: No envelop application.
        // TODO: No trigger.
        // TODO: No externally controlled gain application.
        // -----------------------------------------------------
        let play = || {
            ...
            if let Some((effect_id, _count)) = ff_working_state.played_effect {
                ...
                {
                    let normalized_level = *level as f32 / ff_value_span_f32;
        ...
        loop {
            ...
            if let Ok(events) = platform_device_api.device_mut().fetch_events() {
                ...
                play();
                // let _ = std::thread::sleep(Duration::from_secs_f32(0.0001));
                continue;
            }
            //------------------------------------------------------------------
...
impl VirtualJoystickManager {
    ...
    pub(crate) fn create_virtual_joystick(
        ...
        let joystick = VirtualJoystick::new(name, config, self.debug, self.debug_ff)?;
        self.joysticks
            .lock()
            .unwrap()
