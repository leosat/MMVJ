...
//-------------------------------------------------------------
pub trait Widen {
    type Type: Add<Output = Self::Type>
    ...
    <T as Widen>::Type: std::convert::From<T>,
{
    ...
    pub(crate) fn denormalize_from_unit_domain<FloatT>(
        &self,
        normalized: FloatT,
    ...
    pub(crate) fn normalize_to_unit_domain(&self, value: T) -> f32 {
    ...
    {
        let normalized = input_range.normalize_to_unit_domain(value);
        self.denormalize_from_unit_domain(normalized, allow_extrapolative)
    }
