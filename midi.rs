...
impl MidiManager {
    ...
    fn parse_midi_message(data: &[u8], device_name: &str, debug: bool) -> Option<MidiMessage> {
        let (status, data1, data2) = match data {
            ...
            _ => {
                if debug {
                    log::warn!("Will not process midi message more than 3 bytes in size {data:#?}");
                }
                return None;
            }
        };
    ...
    pub(crate) fn midi_type_matches(
        ...
        match (msg_type, spec_type) {
            (MidiMessageType::PitchWheel, SpecType::PitchWheel) => true,
            (MidiMessageType::ControlChange, SpecType::ControlChange) => true,
            (MidiMessageType::NoteOn, SpecType::Note) => true,
            (MidiMessageType::NoteOn, SpecType::NoteOn) => true,
            (MidiMessageType::NoteOff, SpecType::Note) => true,
            (MidiMessageType::NoteOff, SpecType::NoteOff) => true,
            (MidiMessageType::Aftertouch, SpecType::Aftertouch) => true,
            (MidiMessageType::ProgramChange, SpecType::ProgramChange) => true,
            _ => false,
        }
    }
...
impl MidiLearnMode {
    ...
    fn process_learn_message(&mut self, msg: &MidiMessage) {
        ...
        if !device_controls.contains_key(&control_key) {
            ...
            device_controls.insert(control_key, serde_yaml::Value::Null);
        }
    }
}
